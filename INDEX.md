...
| `raised-razed/` | split-file | Raised/Razed documentary page (Vinegar Hill, Charlottesville) — consolidated from 32 stacked WordPress sections into one Code Block (namespace `vpm-rr-`). Two blocking known issues in README: a 404 headshot (shipped with a placeholder) and an image-only press card with no headline |
