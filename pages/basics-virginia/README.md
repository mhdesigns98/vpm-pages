...
All `font-size` and spacing values use `px` on purpose. `rem` resolves against the host document
root, which Brightspot and WordPress set differently, so `rem` values render at the wrong size in
the CMS — even inside Shadow DOM. Keep new values in `px`.
